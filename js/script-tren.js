...
document.addEventListener("DOMContentLoaded", () => {
        ...
        plugins: {
          ...
          tooltip: {
            callbacks: {
              ...
              label: (context) =>
                `${context.dataset.label}: ${context.formattedValue} ${indicatorData.unit || ""
                }`,
            },
          },
        },
  ...
  fetch("data/data-tren.json")
    ...
    .then((data) => {
      ...
      if (
        !allData.indicators["indeks-gini"] ||
        !allData.indicators["penduduk-miskin"] ||
        !allData.indicators["kelasifikasi"]
      ) {
        console.warn(
      ...
      const locations =
        data.locations || Object.keys(data.indicators.gizi.data); // Fallback if locations array is missing
